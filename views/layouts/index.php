  ...
  50% {
    ...
    <body class="defult-home">
      ...
      <!--  </div> -->
        ...
        <!--popup-->
        <?php
        $activePopup = \app\models\Popup::findOne(['status' => 1]);
        if ($activePopup): 
        ?>
        <div id="boxes">
            <div style="top: 50%; left: 50%; display: none;" id="dialog" class="window">
                <div id="san">
                    ...
                    <img src="<?= Yii::getAlias('@web/') . $activePopup->image_url ?>" width="100%">
                </div>
            </div>
                  ...
                  <div class="main-menu"> <a class="rs-menu-toggle rs-menu-toggle-close"><i class="fa fa-bars"></i>Menu</a>
                    <nav class="rs-menu">
                      <ul class="nav-menu">
                        ...
                        <li><a href="<?php echo  Yii::$app->request->baseUrl;?>/client/login" style="color: #ffde00; font-weight: bold;">Login / Sign Up</a></li>
                      </ul>
                    </nav>
                  </div>
                </div>
      ...
      <!--  <script src="<?php echo Yii::getAlias('@web') . '/web/assets/' ?>js/jquery.mb.YTPlayer.min.js"></script> -->
        ...
        $(document).ready(function(){
            ...
            if(token) {
                // Find Login Link
                $('a[href*="/client/login"]').each(function() {
                    var $li = $(this).parent();
                    
                    // Change Login to My Account
                    $(this).text('My Account');
                    $(this).attr('href', '<?= Yii::$app->request->baseUrl ?>/client/dashboard');
                    $(this).css('color', '#ffde00'); // Visibility
                    
                    // Add Logout Button
                    var $logoutLi = $('<li><a href="#" id="logoutBtn" style="color: #ffde00; font-weight: bold;">Logout</a></li>');
                    $li.after($logoutLi);
                });
...
$(document).ready(function() {	
    var id = '#dialog';
    if ($(id).length) { // Only run if popup exists
