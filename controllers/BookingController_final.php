            ...
            if ($amount == $data['Booking']['amount']) {
                ...
                if($model->save()){
                    ...
                    //var_dump($model); die;
                        ...
                        // SMS disabled for booking creation - only payment confirmation SMS from WebhookController will be sent
                        // try {
                        //     $this->sendSms($model->phone,$message);
                        //     Yii::info("SMS sent successfully to {$model->phone} for booking {$model->id}");
                        // } catch (\Exception $e) {
                        //     Yii::error("Failed to send SMS to {$model->phone}: " . $e->getMessage());
                        // }
...
public function actionUpdateStatus()
{
    ...
    // Use existing OTP if already generated by webhook, otherwise generate new one
    if (empty($booking->otp)) {
        $otp = rand(1000,9999);  
        $booking->otp = $otp;
    } else {
        $otp = $booking->otp; // Use existing OTP from webhook
    }
    ...
    // SMS disabled for final page - only webhook SMS will be sent to avoid duplicates
